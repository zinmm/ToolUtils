package com.zin.toolutils.file;
...
public class FileUtils {
    ...
    public File write2SDFromInput(String fileName, String dir, InputStream input) {
        ...
        try {
            ...
            LogcatUtil.getInstance().error("写数据异常: " + e.getMessage(), FileUtils.class);
        } finally {
            try {
                output.close();
            } catch (Exception e2) {
                LogcatUtil.getInstance().error(e2.getMessage(), FileUtils.class);
            }
        }
        return file;
    }
