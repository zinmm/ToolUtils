...
class ZinSerializer implements Serializer {
  ...
  ZinSerializer(LogInterceptor logInterceptor) {
    this.logInterceptor = logInterceptor;
  }

  @Override public <T> String serialize(String cipherText, T originalGivenValue) {
    ZinUtils.checkNullOrEmpty("Cipher text", cipherText);
    ZinUtils.checkNull("Value", originalGivenValue);
  ...
  @Override public DataInfo deserialize(String serializedText) {
    ...
    if (firstElement != null && firstElement.length() != 0) {
      try {
        ...
        logInterceptor.onLog("ZinSerializer -> " + e.getMessage());
      }
    }
